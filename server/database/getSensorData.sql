CREATE OR REPLACE FUNCTION getSensorData( sensr_id VARCHAR(50), cut_off_date TIMESTAMP default null, capture_date TIMESTAMP default null) returns
...
begin
	if cut_off_date is not null then
    ...
    elsif capture_date is not null then
    	RETURN QUERY SELECT sd.temp, sd.ph, sd.tds, sd.ec, sd.sal, sd.sg, sd.created_date FROM sensor_data as sd
    				 WHERE sd.sensor_id = sensr_id and sd.created_date	< capture_date order by sd.created_date desc limit 1;
    else
    	RETURN QUERY SELECT sd.temp, sd.ph, sd.tds, sd.ec, sd.sal, sd.sg, sd.created_date FROM sensor_data as sd
